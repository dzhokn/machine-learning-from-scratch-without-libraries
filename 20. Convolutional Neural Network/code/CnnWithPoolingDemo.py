...
class CnnWithPoolingDemo:
    ...
    def run():
        ...
        # Create the network.
        network = [
            Convolutional((1, 28, 28), 3, 5),       # Convolutional layer with 5 kernels of size 3x3. The output shape will be (5, 26, 26).
            Relu(),                                 # We always use an activation function after a convolutional layer
            MaxPool(2, 2),                          # Max pooling layer with region size of 2 and stride size of 2. The output shape will be (5, 13, 13).
            Reshape((5, 13, 13), (5 * 13 * 13, 1)), # Reshape the output of the convolutional layer to a 2D array
            Dense(5 * 13 * 13, 2),          # Dense layer with 2 output neurons (0 or 1)
            Sigmoid(),                              # We always use an activation function after a dense layer
        ]

        # Train the network.
        CnnWithPoolingDemo.train(
            network,
        ...
        CnnWithPoolingDemo.test_accuracy(network, X_test, Y_test)
    ...
    def train(network, loss, loss_prime, x_train, y_train, epochs = 1000, learning_rate = 0.01, verbose = True):
        ...
        for e in range(epochs):
            ...
            for x, y in zip(x_train, y_train):
                # forward
                output = CnnWithPoolingDemo.predict(network, x)
